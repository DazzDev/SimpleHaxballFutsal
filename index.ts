...
export const adminAuthList: Set<string> = new Set(fs.readFileSync("adminlist.txt", "utf8").split("\n").map((line: string) => line.trim()));
export const badWordList: Set<string> = new Set(fs.readFileSync("badwords.txt", "utf8").split("\n").map((line: string) => line.trim()));
...
HaxballJS.then((HBInit) => {
  room = HBInit({
    ...
    token: "thr1.AAAAAGPjwQUsHYByErdDZA.AYdO9zxfod8", //https://haxball.com/headlesstoken
  });
